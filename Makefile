...
help:
	...
	@echo "  make test-e2e         - Run e2e tests (requires live HA)"
...
test-e2e:
	HA_E2E=1 uv run pytest tests/e2e/ -v -m e2e

test-cov:
	uv run pytest tests/ -m "not e2e" --cov=src/ha_blade_mcp --cov-report=term-missing -v
...
# Running
run:
	uv run home-assistant-blade-mcp
