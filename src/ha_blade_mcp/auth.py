...
Bearer token auth for remote/tunnel access. Set ``HA_MCP_API_TOKEN`` env var.
...
def get_bearer_token() -> str | None:
    ...
    token = os.environ.get("HA_MCP_API_TOKEN", "").strip()
...
class BearerAuthMiddleware:
    ...
    When ``HA_MCP_API_TOKEN`` is set, every request must carry a matching
